...
NatisCoin Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadRPCServer : Remote procedure call handler, listens on port 7713 for connections and services them.
...
However, in open source software such as NatisCoin Core, where everyone uses
...
pay attention to for reviewers of NatisCoin Core code.

General NatisCoin Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: NatisCoin Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against NatisCoin Core so that they can be integrated
