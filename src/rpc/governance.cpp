...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in natiscoin.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("natiscoin.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* NatisCoin features */
    { "natiscoin",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "natiscoin",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "natiscoin",               "gobject",                &gobject,                true,  {} },
    { "natiscoin",               "voteraw",                &voteraw,                true,  {} },
