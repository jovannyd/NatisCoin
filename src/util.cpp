...
const char * const BITCOIN_CONF_FILENAME = "natiscoin.conf";
const char * const BITCOIN_PID_FILENAME = "natiscoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "natiscoin" is a composite category enabling all NatisCoin-related debug output
                if(ptrCategory->count(std::string("natiscoin"))) {
...
#else
    const char* pszModule = "natiscoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NatisCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NatisCoin
    // Mac: ~/Library/Application Support/NatisCoin
    // Unix: ~/.natiscoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NatisCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/NatisCoin";
#else
    // Unix
    return pathRet / ".natiscoin";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty natiscoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override natiscoin.conf
