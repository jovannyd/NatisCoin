...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("natiscoin"))
        return false;
...
bool parseBitcoinURI(QString uri, SendCoinsRecipient *out)
{
    ...
    if(uri.startsWith("natiscoin://"))
    {
        uri.replace(0, 12, "natiscoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "NatisCoin.lnk";
}
...
{
    return GetAutostartDir() / "natiscoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("NatisCoin-Qt") + " " + tr("version") + " " +
    ...
    setWindowTitle(tr("NatisCoin-Qt"));
